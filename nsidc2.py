...
def c_m_ratio(ds_tb, x, y, x_off, y_off, cmode='Max'):
    if cmode == 'Corr':
        ...
        with open('/Users/lonem/OneDrive/Documenten/Lund University/THESIS/Scripts/cpoi_chikwawa.txt') as f:
...
def optwindow(Q, points_x, wsizes):
    '''
    This function finds the optimal size of the window in which the C cell is 
    searched for, by doing an analysis of association. Calculates the correlation between
    the original data and the CM-ratio in different windows.
    Inputs: dataset with Tb data, dataset with Q data, points of interest, window sizes list
    '''
    corr=[]
    for i in range(len(wsizes)):
        rat, cal, mes = Cmloop(years, points_x, wsizes[i])
        #Fill gaps consisting of a single nan-values (bigger gaps remain)
        ratseries = nanfill(rat)
        #Apply Savitzky Golay filter on ratio
        ratfilt = savitzky_golay(ratseries, 9, 3)
        coef = ma.corrcoef(ma.masked_invalid(ratfilt), ma.masked_invalid(Q))
        corr.append(coef[0,1])
    output = np.vstack([wsizes, corr])
    return output
...
def Cmloop(years, points_x, size):
    ...
    for i in range(len(years)):
        year = years[i]
        endofline = 'DMSP__1992.nc'
        try:
            fns = glob.glob(endofline)
        except:
        ...
        for j in range(len(points_x)):
            # compute CM ratio
            C, M, ratio, rat, cal, mes, time = c_m_ratio(ds['TB'], points_x[j], points_y[j], size, size)
        ratseries.append(rat)
        cseries.append(cal)
        mseries.append(mes)
    return np.asarray(list(flatten(ratseries))), np.asarray(list(flatten(cseries))), np.asarray(list(flatten(mseries)))
